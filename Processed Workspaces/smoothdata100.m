...
CYS=smooth(mocaptime,CPos(2,:),40,'sgolay',2);
...
for i=1:datasize-1
    ...
    RT=[cos(headingmocapSI(i)) -sin(headingmocapSI(i));sin(headingmocapSI(i)) cos(headingmocapSI(i))];
    dSR=RT'*dS;
    longvelocitymocapI(i)=dSR(1)/(time(i+1)-time(i));
    latvelocitymocapI(i)=dSR(2)/(time(i+1)-time(i));
    speedmocapI(i)=sign(longvelocitymocapI(i))*norm(dS)/(time(i+1)-time(i));
    sideslipmocapI(i)=atan2(dSR(2),dSR(1));
...
speedmocapIS=smooth(speedmocapI,50,'sgolay',2)';
longvelocitymocapIS=smooth(longvelocitymocapI,50,'sgolay',2)';
latvelocitymocapIS=smooth(latvelocitymocapI,50,'sgolay',2)';
sideslipmocapIS=smooth(sideslipmocapI,50,'rloess')';
yawratemocapIS=smooth(yawratemocapI,50,'sgolay',2)';
...
for i=1:datasize-1
    accellongmocapI(i)=(longvelocitymocapIS(i+1)-longvelocitymocapIS(i))/(time(i+1)-time(i));
    accellatmocapI(i)=(latvelocitymocapIS(i+1)-latvelocitymocapIS(i))/(time(i+1)-time(i));
...
processeddata=[time';posmocapI(1,:);posmocapI(2,:);distancemocapI;...
