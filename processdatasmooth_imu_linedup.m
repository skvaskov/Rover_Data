...
%and I will manually line up the angular velocities
%%
...
time=imutimeraw(imustart:end)-imutimeraw(imustart)';
...
shiftedrctime=rctimeraw-rctimeraw(rcstart);
shiftedmocaptime=(mocaptime-mocaptime(mocapstart))+.05;
%%
%get interpolated position, and acceleration data
accellongimuCI=accellongimuC(imustart:end);
accellatimuCI=accellatimuC(imustart:end);
posmocapI=zeros(datasize,2);
headingmocapI=zeros(1,datasize);
headingmocapSI=zeros(1,datasize);
yawrateimuI=angvZ(imustart:end)';
steeringrcI=zeros(1,datasize);
throttlercI=zeros(1,datasize);
steeringrcSI=zeros(1,datasize);
throttlercSI=zeros(1,datasize);
for i=1:datasize
    ...
    if isnan(posmocapI(i,1)) || isnan(yawrateimuI(i))
        ...
        throttlercI=throttlercI(1:datasize);
        steeringrcI=steeringrcI(1:datasize);
        throttlercSI=throttlercSI(1:datasize);
        steeringrcSI=steeringrcSI(1:datasize);
        str=[num2str(i) ' nan']
        break
    end
...
hold on
plot(posmocapI(1,:),posmocapI(2,:),'g')
ylabel('position')
hold off
...
plot(time,latvelocitymocapIS,'m')
ylabel('Veocity')
hold off
...
hold on
plot(time,throttlercSI,'r')
plot(time,steeringrcI,'g')
plot(time,steeringrcSI,'m')
xlabel('Inputs')
hold off
